// load is a script that mainly is used for making the sprite offsets align properly in game.

sprite_change_offset("idle", 77, 84);//
sprite_change_offset("idle_air", 71, 80);//
sprite_change_offset("hurtbox", 77, 84);// standard hurtbox
sprite_change_offset("hurtbox_crouch", 56, 84);// crouching hurtbox
sprite_change_offset("hurtbox_air", 77, 80);// airborne hurtbox
sprite_change_offset("hurt", 71, 85);//
sprite_change_offset("hurtground", 75, 80);//
sprite_change_offset("bighurt", 45, 75);//
sprite_change_offset("spinhurt", 65, 75);//
sprite_change_offset("uphurt", 60, 72);//
sprite_change_offset("downhurt", 30, 65);//
sprite_change_offset("bouncehurt", 60, 72);//
sprite_change_offset("crouch", 80, 102);//
sprite_change_offset("walk", 85, 85);//
sprite_change_offset("walkturn", 80, 82);
sprite_change_offset("dash", 85, 72);//
sprite_change_offset("dashstart", 83, 82);//
sprite_change_offset("dashstop", 100, 80);
sprite_change_offset("dashturn", 80, 71);

sprite_change_offset("jumpstart", 84, 98);//
sprite_change_offset("jump", 84, 80);//
sprite_change_offset("doublejump", 80, 80);//
sprite_change_offset("walljump", 32, 62);
sprite_change_offset("pratfall", 70, 75);
sprite_change_offset("land", 86, 64);//
sprite_change_offset("landinglag", 86, 64);//

sprite_change_offset("parry", 32, 62);//
sprite_change_offset("roll_forward", 32, 62);
sprite_change_offset("roll_backward", 32, 62);
sprite_change_offset("airdodge", 83, 82);
sprite_change_offset("waveland", 86, 64);
sprite_change_offset("tech", 32, 62);

sprite_change_offset("jab", 64, 94);
sprite_change_offset("dattack", 64, 94);
sprite_change_offset("ftilt", 104, 130);
sprite_change_offset("dtilt", 58, 64);
sprite_change_offset("utilt", 87, 108);
sprite_change_offset("nair", 82, 98);
sprite_change_offset("fair", 78, 81);
sprite_change_offset("bair", 174, 95);
sprite_change_offset("uair", 91, 144);
sprite_change_offset("dair", 85, 86);
sprite_change_offset("fstrong", 64, 94);
sprite_change_offset("ustrong", 98, 160);
sprite_change_offset("dstrong", 120, 148);
sprite_change_offset("nspecial", 64, 94);
sprite_change_offset("fspecial", 64, 94);
sprite_change_offset("uspecial", 64, 94);
sprite_change_offset("dspecial", 82, 84);
sprite_change_offset("taunt", 32, 62);
sprite_change_offset("phone_open", 32, 64);

sprite_change_offset("plat", 64, 94);

sprite_change_offset("nspecial_proj", 64, 94);

sprite_change_offset("mirror", 32, 32);
sprite_change_offset("mirror_mask", 32, 32);
sprite_change_offset("0_mirror_hit", 14, 34);
sprite_change_offset("1_mirror_hit", 22, 34);
sprite_change_offset("2_mirror_hit", 34, 26);
sprite_change_offset("3_mirror_hit", 34, 34);
sprite_change_offset("4_mirror_hit", 30, 34);
sprite_change_offset("5_mirror_hit", 34, 22);
sprite_change_offset("6_mirror_hit", 34, 10);
sprite_change_offset("7_mirror_hit", 22, 22);

sprite_change_offset("fair_hit", 27, 17);
sprite_change_offset("utilt_hit", 32, 6);